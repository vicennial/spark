...
class ArtifactManager(userContext: proto.UserContext, channel: ManagedChannel) {
  // GRPC supports RPC sizes up to 4 MB. Using the midpoint recommendation of 32KiB for chunk size
  // as specified in https://github.com/grpc/grpc.github.io/issues/371.
  ...
  /**
   * Ensure that all artifacts added to this point have been uploaded to the server,
   * and are ready for use. This method will fail if it finds any problem
   * with the outstanding requests.
   */
  private[client] def ensureAllArtifactsUploaded(): Unit = {
    addArtifacts(classFinders.asScala.flatMap(_.findNewClasses()))
  }
   ...
   * are packaged into a single [[proto.AddArtifactsRequest]].
   */
  private def addBatchedArtifacts(
    ...
    artifacts.foreach { artifact =>
      // TODO: remove base storage trait
      val in = artifact.storage.asInstanceOf[LocalData].stream
      try {
  ...
  /**
   * Add a artifact in chunks to the stream. The artifact's data is spread out over
   * multiple [[proto.AddArtifactsRequest requests]].
   */
  private def addChunkedArtifact(
    ...
    val in = dep.storage.asInstanceOf[LocalData].stream
    try {
      // First RPC contains the name and the first chunk of data.
      // Subsequent RPCs don't contain a name, but do contain a chunk of data.
      val artifactBuilder = builder.getChunkBuilder
      var chunk = ByteString.readFrom(in, chunkSize)
      artifactBuilder.setName(dep.path.toString)
      do {
        artifactBuilder.setData(chunk)
        stream.onNext(builder.build())
        chunk = ByteString.readFrom(in, chunkSize)
        artifactBuilder.clearName()
      } while (!chunk.isEmpty)
    } catch {
      case NonFatal(e) =>
        stream.onError(e)
...
trait ClassFinder {
  def findNewClasses(): Iterator[Artifact]
}
